...
__all__ = ['Test_CelebAGeneration_WACM']

from wt import *

class Test_CelebAGeneration_WACM():
    ...
    def test(self):
        ...
        states = torch.load(os.path.join(self.args.log, 'checkpoint_160000.pth'), map_location=self.config.device)
        ...
        for z in range(1):
            for c in range(3000):
                ...
                x0 = nn.Parameter(torch.Tensor(samples * batch_size, 12, (int)(self.config.data.image_size/2), (int)(self.config.data.image_size/2)).uniform_(-1, 1)).cuda()
                x01 = x0.clone()
                ...
                for idx, sigma in enumerate(sigmas):
                    ...
                    for step in range(n_steps_each):
                        ...
                        x0_b = torch.tensor(idwt(x0[0, 0, ...] * 2, x0[0, 1, ...] * 2 - 1, x0[0, 2, ...] * 2 - 1,
                                                 x0[0, 3, ...] * 2 - 1))
                        x0_g = torch.tensor(idwt(x0[0, 4, ...] * 2, x0[0, 5, ...] * 2 - 1, x0[0, 6, ...] * 2 - 1,
                                                 x0[0, 7, ...] * 2 - 1))
                        x0_r = torch.tensor(idwt(x0[0, 8, ...] * 2, x0[0, 9, ...] * 2 - 1, x0[0, 10, ...] * 2 - 1,
                                                 x0[0, 11, ...] * 2 - 1))
